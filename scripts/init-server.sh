...
term_handler() {
	...
	PID=$(pgrep -of "/opt/wine-stable/bin/wine $server/VRisingServer.exe")
	if [[ -z $PID ]]; then
...
status_steamcmd_game=1
echo " "
echo "Updating V-Rising Dedicated Server files..."
echo " "
while [ $status_steamcmd_game -ne 0 ]; do
...
box64 /opt/wine-stable/bin/wine "$server/VRisingServer.exe" -persistentDataPath $data -logFile "/tmp/$logfile" 2>&1 &
